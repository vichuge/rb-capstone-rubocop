...
describe Rubo do
  describe '.right-spaces' do
    it "return an error if exist right spaces at the end line" do
      expect(Rubo.right_spaces('./examples/inside_example/worst.rb','def my_method   ',2)).to eql("./examples/inside_example/worst.rb line:3 'Right blank space(s) in the line.'\n")
    end
  ...
  describe '.semicolon' do
    it "return error because exist a semicolon at the end line" do
      expect(Rubo.semicolon('./examples/inside_example/bad.rb','p "puts";',1)).to eql("./examples/inside_example/bad.rb line:2 'Semicolon is not neccesary'\n")
    end
  ...
  describe '.nav_operator' do
    it "return error because nav operator has blank-spaces between the symbol (left)" do
      expect(Rubo.nav_operator('bad.rb','foo &.bar', 25)).to eql("bad.rb line:26 'No space required between navigation operator'\n")
    end
  ...
  describe '.space_between_elements' do
    it "return error because a blank-space between elements is required" do
      expect(Rubo.space_between_elements('bad.rb','[ 1,2, 3 ]',35)).to eql("bad.rb line:36 'Space required between separator and array element'\n")
    end
  ...
  describe '.exponent' do
    it "return error if the exponent and his elements has an space between them (left)" do
      expect(Rubo.exponent('bad.rb','e = m * c **2',145)).to eql("bad.rb line:146 'No space required between an exponent operator'\n")
    end
